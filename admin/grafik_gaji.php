...
if(isset($_POST['tahun'])){
	...
	$sql = "select tahun from gaji where tipe='Bulanan' and tahun='$tahun' group by tahun";
	$tipe = "Tahun $tahun";
}else{
	$sql = "select tahun from gaji where tipe='Bulanan' group by tahun";
	$tipe = "Seluruh Data";
}
?>
<div class="col-lg-12">
    <h1 class="page-header">Grafik Gaji Bulanan <small><?=$tipe;?></small></h1>
</div>
	  ...
	  data: [
	    ...
	    <?php
		    ...
		    while($dt = mysql_fetch_array($qt)){
		    	$qb = mysql_query("select bulan from gaji where tipe='Bulanan' and tahun='".$dt['tahun']."' group by bulan");
		    	while($dbl = mysql_fetch_array($qb)){
		    		$q = mysql_query("select sum(total) as total from gaji where tipe='Bulanan' and bulan='".$dbl['bulan']."' and tahun='".$dt['tahun']."'");
